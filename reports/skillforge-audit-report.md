...
**Update (2026-06-13):** PRIV-003 and OPS-002 are now both ✅ resolved and verified live (commit `3f3dcd7`, Netlify deploy `6a2cbc3e0a3cf4000898d50b`). **REL-001 also closed** (resolved on path (a) — committed web/PWA per ADR-0004; all user-facing positioning already aligned). The decision is unchanged — **still NO GO for public release** — because **COMP-001** (no ToS, no self-serve cloud erasure) alone is a sufficient P0 blocker. **BRAND-002 also ✅ resolved** (premium brand fonts self-hosted via `@fontsource-variable`; no Google Fonts origin, CSP untouched, verified zero third-party font hits locally). AUTH-001 / GROW-001 remain open. A new finding, **OPS-003** (Supabase refresh-token retry loop), was logged during today's live verification.
...
|---|---|---|
| **Release Readiness (public)** | **60/100** | 2 of 3 public-P0s now ✅ resolved + verified live (PRIV-003, OPS-002); **COMP-001** (no ToS / no self-serve erasure) is the sole remaining public-P0 and alone blocks GO. REL-001 ✅ closed (web/PWA committed, ADR-0004); BRAND-002 ✅ resolved (fonts self-hosted). AUTH-001, GROW-001, and new OPS-003 still open. |
