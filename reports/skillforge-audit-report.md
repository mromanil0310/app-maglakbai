...
_Last updated: 2026-06-13 (sprint 44 follow-up: all sprint-43/44 fixes committed (`3f3dcd7`) and deployed to production. **OPS-002 ✅ fully resolved + verified live** — PostHog key set in Netlify, CSP fixed, redeployed, and `session_started`/`screen_viewed`/`retention_d1`/`retention_d7`/`client_error` all confirmed arriving in PostHog from the live site. PRIV-003 remains ✅ resolved. REL-001 ✅ closed (committed web/PWA per ADR-0004; all user-facing positioning already aligned). BRAND-002 ✅ resolved (brand fonts self-hosted via `@fontsource-variable` — no Google Fonts origin, tight CSP unchanged, verified zero third-party hits locally). **COMP-001 🔶 resolved in code** (Terms of Service live + reachable pre-signup; full-erasure Delete Account via a `delete-account` Edge Function — code-complete + UI-verified, needs one owner deploy step). **OPS-003 ✅ investigated + closed** (Supabase refresh-token retry loop traced through the `@supabase/auth-js` source — confirmed it never reaches `client_error`/PostHog and is intentional self-healing GoTrue resilience, not a defect). 114 tests, NO GO public still stands until COMP-001's Edge Function is deployed + verified; then only P1s AUTH-001/GROW-001 remain.)_
...
#### OPS-003 — Supabase refresh-token retry loop ✅ Investigated + closed (sprint 44, 2026-06-13)
- **Priority:** ~~P1~~ → informational · **Severity:** ~~🟠 High~~ → 🟢 None (debunked) · **Category:** Ops / Auth / Stability
- **Original finding:** During the OPS-002 live-verification pass on production (`fascinating-kitten-b6a79d.netlify.app`), the Supabase JS client's `GoTrueClient._refreshAccessToken` / `_callRefreshToken` entered a continuous retry loop, throwing `TypeError: Failed to fetch` roughly every 1-4 seconds — 7+ consecutive failures observed in one browsing session.
- **Investigation (sprint 44):** Traced the exact call path in the installed `@supabase/auth-js` source (`node_modules/@supabase/auth-js/dist/module/GoTrueClient.js`):
  1. `_autoRefreshTokenTick()` runs every `AUTO_REFRESH_TICK_DURATION_MS` (30s) and, while a session is within its expiry margin, calls `_callRefreshToken` → `_refreshAccessToken` → `_handleRequest`.
  2. When `fetch()` itself throws (network/offline/CORS → `TypeError: Failed to fetch`), `_handleRequest` does `console.error(e)` and wraps it as `AuthRetryableFetchError`. `_refreshAccessToken`'s `retryable()` then backs off at 200ms/400ms/800ms/1.6s/3.2s/6.4s/12.8s **within that same 30s tick** — exactly matching the observed "every 1-4s, 7+ times" pattern.
  3. Because the error is *retryable* (network-class, not a 4xx from the server), `_callRefreshToken` deliberately does **not** call `_removeSession()` / fire `SIGNED_OUT` — by design, so a brief network blip (tab backgrounded, laptop sleep) self-heals on the next successful tick via `TOKEN_REFRESHED`, with no user-visible disruption.
  4. **Every one of these failures is a `console.error()` call inside `@supabase/auth-js` — none are thrown to `window.onerror` or rejected as `unhandledrejection`.** OPS-001's `installGlobalErrorHandlers()` (`src/utils/errorMonitor.ts`) only listens for those two events. **Confirmed: this never produces a `client_error` event — the "floods PostHog" risk in the original finding was incorrect.**
  5. CSP is not implicated — `connect-src` already allows `https://*.supabase.co wss://*.supabase.co` (`netlify.toml`).
  6. Live-checked this preview's browser: zero `sb-*-auth-token` keys in localStorage and no retry loop — consistent with the original finding being a stale session in that one QA browser, not a general defect.
- **Residual (accepted, no code change):** if a browser's refresh endpoint is *permanently* unreachable (e.g. a deleted/paused Supabase project), the app stays in a silent "signed in but can't sync" state with periodic console noise. Cloud backup is non-blocking for all app functionality, and Settings → Sign Out / Reset already recovers it manually — not worth a dedicated watchdog for an edge case this rare.
- **Status:** ✅ Closed — investigated, debunked as a `client_error`/PostHog risk, confirmed to be intentional self-healing `@supabase/auth-js` resilience. No code change.

### Public-release scorecard (0–100, board scale)

| Score | Value | One-line basis |
|---|---|---|
| **Release Readiness (public)** | **68/100** | PRIV-003 + OPS-002 ✅ live; REL-001 ✅ closed; BRAND-002 ✅ resolved; OPS-003 ✅ investigated + closed (non-issue). **COMP-001 🔶 resolved in code** (ToS fully done + verified; full-erasure Delete Account code-complete + UI-verified) — one owner step (deploy the `delete-account` Edge Function) clears the last public-P0. Remaining after that: P1s AUTH-001, GROW-001. |
